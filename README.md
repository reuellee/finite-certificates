...
The [`optimal full-support segment cover`](ai/omreal/DIAG3_PAIR_FULLSUPPORT_SEGMENT_COVER.md) now compresses those 105 exact segments to a proved-minimum 40 while preserving all 10,844 known wall crossings. Forty-nine unique-crossing factors force 34 edges; an exhaustive seven-pattern residue proves that six more are necessary and sufficient. The same audit restores theorem scope: the section-960 and section-550 stars lie wholly on proper supports and therefore contribute zero relative chain generators. They remain compiler diagnostics, while the proof-bearing pair target stays in full support. Global component coverage remains open and the score remains 2/9.
