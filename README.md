...
The compiler now has two proof-producing instances.  The generated [`17-cell exact master-closure object`](ai/omreal/DIAG3_PAIR_MASTER_CLOSURE_NODE_CANARY.md) covers one two-dimensional full-support node disk in row 2599.  The follow-up [`3 x 3 exact multi-box atlas`](ai/omreal/DIAG3_PAIR_MASTER_CLOSURE_MULTIBOX_CANARY.md) glues four no-wall, four one-wall and one transverse two-wall box by 32 exact boundary-word segments, producing an 81-cell regular-CW object.  Its independent verifier replays all 84,840 labelled residual occurrences, all 97,224 extension signatures and all 216 profile-triple ranks, rejects ten hostile corruptions, and keeps both the artificial outer boundary and internal atlas seams distinct from true parent infinity.  These are end-to-end local canaries, not global parent-cell coverage.
