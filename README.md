...
The direct route now has its first coverage-bearing higher-support checkpoint. On supports `(3,1,15)` and `(3,3,7)`, opposite signed parent brackets force `g=i` and `d=g`; both weak parent closures become the same square pyramid `0<=g<=a<=1`, `0<=g<=h<=1`, covered exactly by two rational tetrahedra. The [`four-support gate`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_GATE.md) independently replays all 70 parent brackets on each support and compresses 8,017 mixed restrictions to 22 active interior walls with zero unresolved classes. Its [`bounded fiber projection`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_PROJECTION.md) reduces the arrangement stage to 136 exact `(u,t)` polynomials. The [`complete second projection`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_BASE_PROJECTION.md) factors these to 114 base curves and projects them to 2,554 distinct squarefree univariate polynomials. Exact [`root isolation`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_ROOT_ISOLATION.md) orders all 1,693 interior `t` sections. The [`open-sector lift`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_OPEN_SECTOR_LIFT.md) certifies all 265,962 base cells over the 1,694 open sectors, including the formerly implicit `u=1` boundary audit. The [`simple-section lift`](ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_SIMPLE_SECTION_LIFT.md) completes another 170,610 cells on 1,022 transversal algebraic fibers and leaves exactly 671 harder sections. Those sections, `v` lifting, global gluing, and middle-rank replay remain open, so the score remains 2/9.
...
| 9DVL / realizability | **2/9 proved**; the first two four-support parent domains have exact coverage, a complete projection, 265,962 open-sector base cells, and 170,610 cells on 1,022 simple algebraic sections, but 671 harder sections, `v` lifting, the global closure complex, and both diagonal-three invariant obligations remain open | [`ai/omreal/`](ai/omreal/) |
