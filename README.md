...
The strongest results in the repo, each with exact certificates and an adversarial
review trail:

1. **max f₀(3,5) = 42** — the maximum vertex count of a 3-dimensional zonoboxtope
   with 5 generators, refuting the tightness of Prop. 6.5 and the odd case of
   Conjecture 6.6.1 of Balakin–Cox–Loho–Sturmfels (arXiv:2509.21286) at n = 5.
   132,560 exact cell-wide certificates. [`ai/maxout/`](ai/maxout/), with an
   [arXiv-ready note](ai/maxout/paper/).
2. **Mutation graphs of uniform oriented matroids are connected at every rank for
   n ≤ 9** — including the labelled (4,9) level, where Knauer–Marc suspected a
   counterexample and which they placed beyond present knowledge; the suspected
   counterexample does not exist. The computation also corrects the published
   count of uniform rank-4 classes on 9 elements to **9,276,595** (not 9,276,601).
   [`ai/omgamma/`](ai/omgamma/), with an [arXiv-ready note](ai/omgamma/paper/).
3. **Non-realizability at (4,9) is generic, not generated by a short list of
   obstructions** — ~91% of non-realizable classes contain one of the 24 known
   (4,8) obstructions as a deletion, but the minor-minimal residue extrapolates to
   ~10⁴ templateless classes, in sharp contrast to the 24 symmetric obstructions
   one element down. Includes Proposition R: a non-realizable deletion always
   lifts to a biquadratic final polynomial.
   [`ai/omminor/`](ai/omminor/MINOR_THEORY.md).

**In progress:** the realizability split of all 9,276,595 uniform rank-4 oriented
matroid classes on 9 elements — blank in the literature since 2013 — and with it a
completeness test for biquadratic final-polynomial certificates over the whole
cell ([`ai/omreal/`](ai/omreal/)).
...
**Oriented matroids / discrete geometry**
...
| Single-element extension atlas theorem (SEEAT), exact 26,112 one-chart capacity at (4,8), and a 5,902-matrix certificate proving four charts do not suffice (atlas width at least 5) | [`ai/omreal/SEEAT.md`](ai/omreal/SEEAT.md) |
