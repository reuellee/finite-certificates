...
In parallel, the [`triple multibox frontier canary`](ai/omreal/DIAG3_TRIPLE_FRONTIER_MULTIBOX_CANARY.md) extends the registered hard presentation from one box to a connected 20-macrobox corridor covered by 35 exact certificate boxes. All 1,400 parent-bracket intervals are strict and the fixed 147-term projection minor is negative on all 35 boxes, so every restricted component meets one of the corridor's 18 artificial outer facets. The first rejected macrobox exactly crosses the genuine parent wall `[3468]=g-a`, identifying the next clipped terminal cell. Independent clean replay and deterministic regeneration pass with 10/10 hostile mutations rejected. This closes no complete row or orbit: the triple residue remains 1,162,302 and the score remains 2/9.
