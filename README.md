...
An exact [`coverage-strategy audit`](ops/team/coverage-theory/ARCHITECTURE_AUDIT.md) now changes the workload priority. The 40-edge retained graph is a forest on 48 chart vertices with eight connected components and cycle rank zero, omitting 130 of the 178 stored charts; compiling its other 38 edges would add labelled trees but no two-cells or component-coverage theorem. A separate exact incidence no-go shows that complete skeleton data alone cannot imply ambient component coverage, while the edge-39 replay exhibits 118 factors with two interior roots and equal endpoint signs. Bulk edge compilation is therefore retained only as a supporting diagnostic, not the primary proof route.

The first [`order-two triangle coverage discriminator`](ops/team/triangle-certificate/README.md) fills the exact barycentric triangle on charts 0, 89, and 113. All 70 parent brackets remain strict. Exact depth-three Bernstein replay partitions all 17,824 candidate factors into 5,665 with certified interior zeros, 12,096 certified empty on the closed triangle, and 63 retained unresolved. Crucially, 77 certified interior-zero factors occur on neither compiled incident edge 27 nor edge 39. A standalone [`independent verifier`](ops/team/triangle-referee/RESULT_HANDOFF.md) reconstructs the result from pinned raw inputs and rejects 10 hostile mutations. The third side, component incidences, global parent-cell coverage, pair injectivity, and the triple obligation remain open; the score stays 2/9.
