...
Usage: python certify.py <r> <n> [max_expand] [prefix]
"""
import json
import sys
import time
...
def main(r, n, max_expand=20000, prefix=None):
    d = f"data/big_{r}_{n}"
    ...
    nedge = 0
    nexp = 0
    for c in range(min(max_expand, st['total_classes'])):
        if hol.full():
            break
        nexp += 1
