...
identity broken), a truncated array, and a stale hash. The first nine ship
with a regenerated, internally consistent manifest --- fresh hashes for the
witness arrays too --- and with the count and mass repaired, so that only a
substantive mathematical check can catch them; the remaining two are
...
at the time of writing}; one is planned, and until it does the way to
