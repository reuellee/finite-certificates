...
standalone checkers accompany the holonomy claims, the
$9{,}276{,}595$-class enumeration, and a mutation spanning tree joining
those classes into a single component; \S\ref{sec:honesty} states
...
\begin{verbatim}
    ...
    python coverage_checker.py --canary --artifact data/coverage_4_9
                                               # 11 sabotages; must REJECT
\end{verbatim}
The last two need the coverage and witness arrays, which are too large for
the repository; \S\ref{sec:honesty} says how to obtain them, and how far
each of these commands reaches.
...
(\S\ref{sec:honesty}), recording for each class but a fixed root a parent
class, the basis mutated, and the voltage of that edge. The same
...
list of $9{,}276{,}595$ class keys with their stabilizer orders. That list
is now a checkable artifact: the keys sorted, each a $126$-bit integer
recording the signs of a chirotope on the bases in colex order, with each
class's stabilizer order; $62$~MB compressed, readable with \texttt{numpy}
and nothing else, beside a manifest fixing the conventions and recording
the \texttt{SHA-256} of every array. Beside it sits an $83$~MB
\emph{reachability witness}: for every class but a fixed root, the row of
the class it was reached from, the colex index $j_c$ of the mutated basis,
the voltage $(\sigma_c,\varepsilon_c,s_c)\in G'$ of that edge, and the
distance to the root.

A standalone checker \texttt{coverage\_checker.py}, importing nothing from
this project and rebuilding the basis order, the Grassmann--Pl\"ucker
conditions, the sign lattice and the group action from scratch, verifies in
exact integer arithmetic that every key is a valid uniform chirotope; that
every key is extremal in its own orbit; that the keys are strictly
increasing, hence pairwise distinct; that every recorded stabilizer order
is the true one, recomputed by exhaustively enumerating the admissible
relabellings rather than by rerunning the generator's pruned search; and
that the orbit masses sum to $1{,}722{,}704{,}635{,}330{,}560$ with a count
of $9{,}276{,}595$. On the witness it verifies that every $\sigma_c$ is a
permutation; that exactly one class is parentless; that
$\mathrm{depth}(c)=\mathrm{depth}(\mathrm{parent}(c))+1$ for all others, so
that the parent map strictly decreases depth and cannot cycle, an
independent pointer-doubling pass confirming that every chain ends at that
one root; and that each edge satisfies, exactly,
\[
  (\sigma_c,\varepsilon_c,s_c)\cdot\chi_c
   \;=\;\mu_{B_{j_c}}\bigl(\chi_{\mathrm{parent}(c)}\bigr).
\]
That identity exhibits the mutant of the parent as a $G'$-translate of the
child; since $\chi_c$ is valid and $G'$ preserves validity, $B_{j_c}$
really is a mutable basis of the parent, and the two classes really are
adjacent in $\Gu^{9,4}$. With the tree structure, every listed class is
therefore joined to the root by a path of certified mutation edges.

This was run in full, not sampled: all $9{,}276{,}595$ rows and all
$9{,}276{,}594$ tree edges, in $853$ seconds on two processes ($702$ for
the catalog checks, $144$ for the tree), all $39$ checks passing. On a $20{,}000$-row sub-artifact --- the rows nearest
the root, which is closed under the parent map and so is a complete
artifact in its own right --- eleven sabotages are each rejected: a
duplicated key, a
non-canonical key, a corrupted stabilizer order, a
Grassmann--Pl\"ucker-invalid key, a re-pointed parent creating a cycle, a
second parentless class, a corrupted voltage, a wrong mutated-basis index,
a parent re-pointed to a different class at the same depth (tree intact,
identity broken), a truncated array, and a stale hash. Each is built at
run time by tampering with a $20{,}000$-class, parent-closed excerpt of
the real arrays --- so the suite needs those arrays, like the full run
above. The first nine carry a regenerated, internally consistent manifest
--- fresh hashes for the witness arrays too --- and have the count and
mass repaired, so that only a substantive mathematical check can catch
them; the remaining two are
deliberately different, the truncation being meant to be caught by the
count and mass arithmetic and the stale hash by the integrity check.

Neither array is tracked in git. Their manifest is, and pins them by the
\texttt{SHA-256} of each raw array buffer. \emph{No archived release exists
at the time of writing}; one is planned, and until then the way to
obtain the arrays is to regenerate them with the shipped code --- about
four hours for the search on four cores, five minutes for the export ---
and to check them against those hashes, which reproduce exactly.
...
does not depend on the target at all: it is what checks (a)--(e) and the
spanning-tree checks above establish. That they are \emph{all} the classes
