...
was not touched); it takes ~75 min single-core and produced exactly
**312 356** classes — matching FMM13's published total independently,
before any realizability work.  The 26 MB catalogue is not committed; the
one-line regeneration is cheaper than the storage.
...
Committed alongside the code: `certs_3_8.jsonl`, `certs_3_9.jsonl` and
`certs_4_8.jsonl` — the certificates behind the three reproduced splits,
i.e. the correctness gate itself, re-checkable by anyone with
`python checkcert.py`.  Per-shard run outputs are not committed; they are
...
cost 3 940 core-hours. It does not have to be one.

### 11.1 The idea

`ai/omgamma/data/coverage_4_9/witness_4_9.npz` is a certified mutation
spanning tree over the entire catalogue, depth 27. For every non-root row
it records a parent, a mutated basis `flip[i]`, and a group element
g = (sigma, eps, gsgn) satisfying, exactly,

```
g . chi_i  =  mu_{B_flip[i]} ( chi_parent[i] )
```

So a class and its parent differ in **one bracket sign**, up to relabelling
and reorientation. If the parent is realized by an integer matrix Z_p, then
realizing the child is not a search: it is **one wall crossing** from a
configuration already in the right place — push bracket `flip[i]` through
zero while the other 125 hold, then undo g.

The pilot had already found this structure the hard way. §6.3: stalled
searches sit at *exactly one* wrong bracket — they have realized a mutant
of the target and cannot cross to it. The tree hands us that mutant,
already realized, for free.

### 11.2 The three prerequisites, verified

`python treewalk.py verify` (all must hold, all do):

* the SHA-256 of all six witness arrays match `MANIFEST.json`;
* the identity `g . chi_i == mu_flip(chi_parent)` holds on **1500/1500**
  random rows, recomputed from decoded sign vectors;
* the matrix action agrees with the chirotope action, and its inverse
  round-trips (25/25 each, checked with exact integer determinants).

### 11.3 Measured

**Walk of the subtree of depth <= 8** (`treewalk.py walk --maxdepth 8`),
4311 rows:

| outcome | count | share | cost |
|---|---|---|---|
| crossed from the parent | 4308 | **99.93%** | **26.1 ms** |
| fell back to a search | 0 | 0% | — |
| unrealized | 3 | 0.07% | — |

End to end **26.5 ms/class**. All 4309 certificates accepted by
`checkcert.py`.

A depth-limited walk only ever sees the shallow part of the tree, so it
cannot by itself say whether the crossing keeps working at depth 27.
`treewalk.py probe` answers that without walking there: sample rows
**uniformly over the whole catalogue**, realize each row's *parent* from
scratch, and attempt exactly the crossing the walk would attempt. If
anything this is pessimistic — a searched parent is a generic point of its
