#!/usr/bin/env python3
"""Fail-closed audit of the cross-diagonal canonical research state."""
...
DATA = HERE / "data"
LEDGER_PATH = DATA / "CANONICAL_RESEARCH_STATE.json"
STATUS_PATH = HERE / "NINE_DIAGONAL_STATUS.md"
...
POINTER = "ai/omreal/data/CANONICAL_RESEARCH_STATE.json"
LEGACY_DIAG3_POINTER = "ai/omreal/data/DIAG3_RESEARCH_DECISION_LEDGER.json"
...
def assert_current_state(ledger: dict) -> None:
    ...
    assert ledger["format"] == "9dvl-canonical-research-state-v1"
...
def assert_status_surface() -> None:
    ...
    present = [heading for heading in headings if text.count(heading) == 1]
    assert len(present) == 1
    assert text.index(present[0]) < text.index("## Result ledger")
    required = (
        ...
        "PIVOT_REQUIRED",
        "no selected\nmathematical target",
        "local roadmap, box, collar, macrobox, or clipped-wall",
        "RETIRED_UNTIL_GLOBAL_INPUTS",
        "theorem-ready global compactification, signed face\n  poset, and restriction matrices",
        "STOP` fail-closed",
    )
    for needle in required:
        assert needle in text, f"missing current status token: {needle}"
...
def main() -> None:
    ...
    algebraic_t_coefficient_endpoint_u_point_v_lift = progress[
        "thirty_third_pair_first_four_support_algebraic_t_coefficient_endpoint_u_point_v_lift"
    ]
    assert algebraic_t_coefficient_endpoint_u_point_v_lift == {
        "status": "PROVED",
        "scope": "exact coefficient, endpoint, fiber-wide-zero, tangency, and compound-resultant v lift for the final algebraic-t algebraic-u point fibers",
        "prior_residual_points": 11_938,
        ...
        "point_signature_catalog_entries": 418,
        "cumulative_algebraic_t_base_cells": 261_571,
        "cumulative_algebraic_t_lifted_cells": 8_390_619,
        "completed_reason_census": {
            "coefficient_endpoint_count_change": 2_888,
            "coefficient_endpoint_equal_count": 1_692,
            "coefficient_only": 3_380,
            "endpoint_count_change": 3_976,
        ...
        "partition_sha256": "4354ba4d3c005cbe0732220d4eeff83f9ada201209f4aa5981c61bd1896e62ad",
        "point_signature_catalog_sha256": "5c8c2de074d13ac41e5ed259f1ad9db02af060d0d9465611700a91283b2adeba",
        "semantic_sha256": "20e32b1164509eb9b0151dc9e18737ade7f53add85e8826bf8bee811958b00bb",
        "artifact_shards": 32,
        "hostile_corruptions_rejected": 32,
        "all_algebraic_t_u_point_v_lifts": "COMPLETE",
        "all_algebraic_t_v_lifts": "COMPLETE",
        "global_gluing_and_closure_data": "NOT_CLAIMED",
        "evidence": [
            "ai/omreal/DIAG3_PAIR_GLOBAL_FOUR_SUPPORT_ALGEBRAIC_T_COEFFICIENT_ENDPOINT_U_POINT_V_LIFT.md",
    ...
    for historical in (completion, closure):
        assert historical["current_decision_ledger"] == LEGACY_DIAG3_POINTER
