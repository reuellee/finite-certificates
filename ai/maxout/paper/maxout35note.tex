...
zonotopes with pairwise parallel generators and independent
...
(conjectured maxima: $60$ and $88$). Searches that are exhaustive for
each fixed sampled direction configuration found no more, but the
space of direction configurations was sampled, not exhausted; whether
the odd-case deficit $2$ persists for all odd $n>3$ is an open
...
copies. With $22$ chambers this gives
...
$w_i>0$ the residual weight; all translation freedom enters through a
...
complete coefficientwise-positive mechanism among ordinary polynomial
...
earlier stage, were wired into every final certificate sweep and
...
(all rank-$3$ oriented matroids on at most eight elements are
realizable \cite[Ch.~8]{OMbook}, so there are no other cells). Hence every generic instance moves, without
