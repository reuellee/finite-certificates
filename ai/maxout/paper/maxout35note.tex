...
(\texttt{ai/maxout} of the repository cited below; the last two need
\texttt{numpy}, \texttt{scipy}, \texttt{sympy}):
\begin{verbatim}
    ...
    python stage2c2_gpt/audit_all_certificates.py  # ~20 min, all
...
the standalone audit and were re-verified at fresh realizations in
review.
...
library: coefficient nonnegativity, all quotient-ring identities of
every explicit certificate re-derived modulo the
Grassmann--Pl\"ucker ideal in exact rational arithmetic (the family
certificates satisfy ordinary polynomial identities needing no
reduction), and specialization at $U_{\mathrm{ref}}$ against an
...
\emph{unverified} model output: the model-produced certificates and
programs are exactly re-checked by the standalone audit, the upper
bound rests on that audited library and the short arguments of
