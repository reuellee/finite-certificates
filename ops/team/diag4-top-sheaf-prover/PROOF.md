...
with coordinates `(t_1,t_2,t_3,u)`. More explicitly, use the four nonmoving
rays as a basis; parent uniformity makes every moving-ray coordinate nonzero.
The residual diagonal projective stabilizer preserving an `f`-line equates
the three non-`f` scales, and preserving the line with distinct apex `h`
equates the remaining scale. Thus only scalar matrices remain, and the
quotient fiber is genuinely four-dimensional. The circuit conditions impose no extra
...
fiber. No global trivialization of the top orientation local system is used:
every fiber already has zero compact-support cohomology in rows below four,
so every total degree at most three vanishes. Fiberwise compactification of
`pi` and proper base change identify the
...
* **Positive:** `123/124/235/147/368` has block `(5,6,8)` dominated by apex
  ...
  in a proper pairwise-incomparable family; the canonical
  `verify_seeat_shatter8.py` replay checks all 256 patterns on all eight
  signatures, including signature index `7` used by this canary.
