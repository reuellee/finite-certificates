...
did produce a complete minimal abstract classification over an oriented base
line with two exterior rays and one exact actual-OM candidate family for the
next topology calculation.
...
## 1. Complete oriented-line minimal abstract classification

The exact abstract search class was a component diagram over a **base line
with two exterior rays**, with:

- connected exterior fibers;
- no births or deaths;
- at most two generic split/merge events; and
- at most two simultaneous component branches.
...
Over the declared oriented base line with two exterior rays, this exactly
excludes every zero- or one-event obstruction and reduces the minimal
two-event class to one signed bit.  It does **not** refute D4-SP: neither gauge
class was derived from a rank-four third-compound realization.
...
The abstract enumeration is complete only for the stated base-line class
with two exterior rays, two events, two components, and no births/deaths.  It
excludes circular or otherwise non-line bases, higher event counts,
