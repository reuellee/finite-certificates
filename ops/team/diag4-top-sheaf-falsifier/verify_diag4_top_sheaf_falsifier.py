...
* It exhausts the smallest abstract signed split--remerge class: connected
  one-dimensional component diagrams with no births/deaths, at most two
  generic component-changing events, and at most two simultaneous branches.
  The only cyclic event word is ``1 -> 2 -> 1``.  All sixteen signed
  attachment matrices are checked exactly over Q and reduced to the two
  orientation-holonomy classes.
...
def signed_minimal_class(payload):
    ...
    # component graph.  These are the complete connected generic shapes in
    # the declared search class; E-V+1 is zero in every case.
    boundary_shapes = {
        "no_event": (2, 1),
        "one_split": (4, 3),
        "one_merge": (4, 3),
    ...
    print("PASS abstract minimality: zero/one events are trees; only 1-2-1 can cycle")
...
def main():
    ...
    print("OUTCOME finite-exact abstract classification; global realizability INCONCLUSIVE")
