...
class Audit:
    def __init__(self,root):
        self.root=root; self.hashes={}; self.cache={}
        for path,digest in PINS.items():
            ...
            require((root/path).read_bytes()==data,"working source drift: "+path)
        ...
        self.upper=np.load(root/'ai/omreal/data/seeat_parent2599_upper178.npz',allow_pickle=False)
        self.shatter=np.load(root/'ai/omreal/data/seeat_parent2599_shatter8.npz',allow_pickle=False)
        ...
        lines=[line.strip() for line in (root/'ai/omgamma/data/cat_4_8.txt').read_text().splitlines() if line.strip()]
...
def main():
    ...
    audit=Audit(root)
    principal=json.loads((root/PREFIX/'OBSTRUCTION.json').read_bytes())
    controls=json.loads((root/PREFIX/'CONTROLS.json').read_bytes())
