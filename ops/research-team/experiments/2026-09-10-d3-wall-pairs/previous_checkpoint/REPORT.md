...
**Independent exact replay accepted the auxiliary results below. Original cohomological injectivity remains open; the theorem ledger is 2/9.**
...
| Guess | Independently checked outcome | Mathematical scope |
|---|---|---|
...
| Every proper signature on actual parents admits a continuous normalized Gordan witness choice near every bad parent. | Exact counterexample: two bad approaches force disjoint limit constraints. | One new actual uniform parent chamber and one realizable proper signature; no three-signature antichain is certified. |
...
Run `python -B replay_checkpoint.py` from the checkpoint root. It authenticates all 15 source files and runs three independently implemented standard-library checkers in a temporary clean copy. The clean replay passes 34 hostile controls. The referee separately reviewed the written continuity lemma, limiting argument, and scope. This is exact arithmetic plus independent deductive review, not proof-assistant certification. Discovery code is included separately; the final replay does not require NumPy, SciPy, or SymPy.

The stronger no-section obstruction is preserved under regular continuous parent chart changes and positive column rescaling. The minimum-norm corollary is stated in the raw-normal gauge; explicit coordinates in the repository's full canonical nine-dimensional gauge are not supplied. See `referee/FINAL_REVIEW.md` for the independent verdict and `falsifier/FINDINGS.md` for the complete proof.
