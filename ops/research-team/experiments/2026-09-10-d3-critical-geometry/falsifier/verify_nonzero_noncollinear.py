...
assert any(gammas['Q']) and any(gammas['R'])
...
relabels={'Q':([2,1,3,4,8,6,7,5],['123','145','246','357']),'R':([2,4,8,1,6,5,3,7],['123','145','246','378'])}
...
def enc(v):
 ...
 return v
out=enc({'verdict':'PASS actual uniform noncollinear vertical-critical point with BOTH gradients nonzero','scope':'Refutes dependent nonzero height gradients imply collinear selected ordinary concurrences, even after excluding individually stationary or height-flat walls. No compact component or global triple-escape counterexample is claimed.','parent':Y,'supports_1based':Ks,'global_factor_kinds':{'P':48,'Q':49,'R':50},'parent_brackets_1based':brackets,'ordinary_normal_matrices':N,'ordinary_concurrences':points,'ordinary_circuit_dependences':circuits,'concurrence_span_rank':3,'all_pairwise_concurrences_distinct':True,'circuit_gradients_8heights':gammas,'direct_normal_determinant_derivatives_8heights':derivatives,'normalized_free_height_labels':[5,6,7,8],'direct_derivatives_4heights':vertical,'vertical_gradient_rank':1,'both_vertical_gradients_nonzero':True,'new_original_obligations':0,'new_triple_source_count':0})
(P/'NONZERO_NONCOLLINEAR_REPLAY.json').write_text(json.dumps(out,indent=2)+'\n')
