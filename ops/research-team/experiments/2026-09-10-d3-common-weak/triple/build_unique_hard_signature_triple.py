...
H=json.loads((root/'UNIQUE_HARD_THREE_BOUNDARY_POINTS.json').read_text());J=H['center'];Y=s.Matrix([[s.Rational(x) for x in r] for r in J['Y']]); triples=list(itertools.combinations(range(8),3));bases=list(itertools.combinations(range(8),4));si=H['supports']
def normal(A,I):
 Q=A[:,I];return s.Matrix([(-1)**(r+3)*Q.extract([j for j in range(4) if j!=r],range(3)).det() for r in range(4)]).T
A=s.Matrix.vstack(*(normal(Y,I) for I in triples));a,d,g=s.symbols('a d g');Z=Y.copy();Z[1,5]=a;Z[1,6]=d;Z[1,7]=g
qs=[s.expand(s.Matrix.vstack(*(normal(Z,triples[k])for k in S)).det())for S in si]
u,v,w=s.symbols('u v w');solution=s.solve([q-t for q,t in zip(qs,[u,v,w])],[a,d,g]);YY=Z.subs(solution)
print('qs',qs,'Jac',s.Matrix([[s.diff(q,z)for z in[a,d,g]]for q in qs]).det(),'solutions',solution,flush=True)
out={'schema':'actual_hard_three_distinct_weak_primal_points_v1','center':J,'parameterized_matrix':[[str(t) for t in r] for r in YY.tolist()],'parameter_coordinates':['u','v','w'],'support_determinants':[str(q)for q in qs],'signatures':[]}
for j in range(3):
 N=A[si[j],:];lam=N.T.nullspace()[0];sig=s.Matrix([s.sign(x) for x in lam]);pp=s.Matrix(H['points'][j]);zeros=[k for k in range(56) if(A*pp)[k]==0];print('block',j,'support',si[j],'zero',zeros,'rel',list(lam),flush=True)
 for exp in range(2,13):
  ...
  sigma=[int(s.sign(x))for x in values];center_values=A*pp;orient=next(sigma[k]*s.sign(center_values[k])for k in range(56)if center_values[k])
...
(root/'ACTUAL_UNIQUE_HARD_THREE_BOUNDARY_SIGNATURES.json').write_text(json.dumps(out,indent=2)+'\n')
