...
H=json.loads((root/'HARD_THREE_BOUNDARY_POINTS.json').read_text());J=H['center'];Y=s.Matrix([[s.Rational(x) for x in r] for r in J['Y']]); triples=list(itertools.combinations(range(8),3));bases=list(itertools.combinations(range(8),4));si=H['supports']
def normal(A,I):
...
(root/'ACTUAL_HARD_THREE_BOUNDARY_SIGNATURES.json').write_text(json.dumps(out,indent=2)+'\n')
