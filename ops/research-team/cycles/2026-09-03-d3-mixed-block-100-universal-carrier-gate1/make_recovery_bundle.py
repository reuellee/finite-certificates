#!/usr/bin/env python3
"""Create and optionally mirror the final mixed-(1,0,0) recovery bundle."""
...
def main() -> None:
    ...
    stem = f"9dvl-d3-mixed-100-carrier-gate1-{commit[:7]}-{DATE}"
    ...
    checked = subprocess.run(["git", "bundle", "verify", str(bundle)], cwd=ROOT, check=True, text=True, capture_output=True)
    closing = HERE / "CLOSING_MANIFEST.json"
    verifier = HERE / "verify_final_closing.py"
    value = {
        "format": "d3-mixed-100-recovery-manifest-v1",
        "cycle_id": HERE.name,
        ...
        "bundle_verify_stdout": checked.stdout.strip(),
        "bundle_verify_stderr": checked.stderr.strip(),
        "github_write": False,
        "google_drive_connector_used": False,
        "native_mirror": {"attempted": False, "available": MIRROR_ROOT.is_dir()},
    }
    manifest.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n", encoding="ascii")
    if not args.no_mirror and MIRROR_ROOT.is_dir():
