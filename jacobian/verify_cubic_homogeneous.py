...
The default mode requires ``cubic_map.py`` to equal the exact reconstruction.
Use ``--regenerate-artifact`` only when intentionally updating that witness.
"""
import argparse
...
ns = read_literal_assignments(
    Path(__file__).with_name("deg3_map.py"),
    {"N", "components", "points"},
)
n = ns['N']
...
# ---- validate or deliberately regenerate the downstream artifact ----
artifact = Path(__file__).with_name("cubic_map.py")
renamed_components = []
for comp in L:
    value = str(comp)
    value = re.sub(r'\bw(\d+)\b', lambda match: 'v%d' % (n + int(match.group(1))), value)
    value = re.sub(r'\bT\b', 'v%d' % (2*n), value)
    renamed_components.append(value)
expected = {
    "N": 2*n + 1,
    "components": renamed_components,
    "points": [[str(c) for c in wv] for wv in wit],
}
...
def render_artifact(values):
    ...
    lines.extend("    %r,\n" % component for component in values["components"])
    lines.append("]\npoints = [\n")
    lines.extend("    %r,\n" % point for point in values["points"])
    lines.append("]\n")
    return "".join(lines)
